...
** Copyright 2010, Tarik Al-Ani <tarik.nzl@gmail.com>
** 
...
//static GpsState  _gps_state[0];
static GpsState  _gps_state[1];
static GpsState *gps_state = _gps_state;

//FIXME specific to freerunner. must have somesort of check to see if GPS is powered on
//#define GPS_POWER_IF 1
...
gps_state_start( GpsState*  s )
{
    ...
    D("ANDREO:D GPS STATE START- %s", __FUNCTION__);    
...
gps_state_init( GpsState*  state )
{
    ...
    do {
        state->fd =  open(DEVICE_TTYS, O_RDWR | O_NONBLOCK); 
    ...
    if ( isatty( state->fd ) ) {
		D("ANDREO:D IS A TTY - %s",__FUNCTION__);

		//TTY SETTINGS
		D("ANDREO:D Test TTY INIT START %s",__FUNCTION__);
		struct termios ios;
		...
		ios.c_cc[VTIME]    = 5;   /* inter-character timer unused */
		ios.c_cc[VMIN]     = 0;   /* blocking read until 9 chars received */

		tcflush(state->fd, TCIFLUSH);
		tcsetattr(state->fd,TCSANOW,&ios);

		D("ANDREO:D Test TTY INIT END %s",__FUNCTION__);
...
freerunner_gps_set_fix_frequency(int freq)
{
    ...
    D("DEBUG freerunner_gps_set_fix_frequency");
    if (!s->init) {
        DFR("%s: called with uninitialized state !!", __FUNCTION__);
        return;
    }
...
{
    GpsState*  s = _gps_state;
    D("DEBUG freerunner_gps_set_position_mode");

    // only standalone supported for now.
/*
    if (mode != GPS_POSITION_MODE_STANDALONE){
        D("DEBUG Running in NON-STANDALONE %d", mode);
	D("DEBUG Changing mode to STAND ALONE");
...
*/
 	D("ANDREO:D DEV POWER - %s", __FUNCTION__);

    do {
        fd =  open(DEVICE_TTYS, O_RDONLY | O_NONBLOCK); ;
...
static void gps_dev_init(int fd)
{
  D(" --- BEGIN gps_dev_init --- %s", __FUNCTION__);
  gps_dev_power(1);

  //set baud rate (tarik)
  
  gps_dev_set_baud_rate (fd, 9600);

  usleep(1000*1000);

  // To set to STOP state
  gps_dev_stop(fd);
  D(" --- END gps_dev_init --- %s", __FUNCTION__);
  return;
